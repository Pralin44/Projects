...
<html>
  ...
  <script src="https://kit.fontawesome.com/2866db5307.js" crossorigin="anonymous"></script>
    ...
    form, .content{
	    ...
	    background: white;
	    border-radius: 0px 0px 3px 4px !important;
    }
    table {
            width: 100%;
    ...
    <body style="background-color: rgb(250, 250, 251);">
        ...
        <main style="margin-left:15rem !important;padding:2px !important;">
            <h1>Hello Admin!!</h1>
            <?php
            // Display all pending pet requests
$sql = "SELECT * FROM pets WHERE status = 'pending'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo "<h2>Pending Pet Requests</h2>";
    echo "<div class='table-container'>";
    echo "<table>";
    echo "<tr><th>ID</th><th>Image</th><th>Name</th><th>Breed</th><th>Description</th><th>Action</th></tr>";
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>".$row["id"]."</td>";
        echo "<td><img src='../".$row["image"]."' alt='Pet Image' width='50'></td>";
        echo "<td>".$row["name"]."</td>";
        echo "<td>".$row["breed"]."</td>";
        echo "<td>".$row["description"]."</td>";
        echo "<td>";
        echo "<form action='accept_pet.php' method='post' style='display: inline-block;'>";
        echo "<input type='hidden' name='pet_id' value='".$row["id"]."'>";
        echo "<input type='submit' value='Accept'>";
        echo "</form>";
        echo "<form action='refuse_pet.php' method='post' style='display: inline-block;'>";
        echo "<input type='hidden' name='pet_id' value='".$row["id"]."'>";
        echo "<input type='submit' value='Refuse'>";
        echo "</form>";
        echo "</td>";
        echo "</tr>";
    }
    ...
    echo "<h2>No pending pet requests</h2>";
}
