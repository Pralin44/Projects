...
class DogBreedClassifier():
    ...
    def print_msg(message, line_break=True):
        if line_break:
            message += '\n'
        sys.stdout.buffer.write(message.encode(sys.stdout.encoding, errors='replace'))
 
    def __init__(self):
        self.model = keras.models.load_model('dog_breed_classifier-main/saved_models/DogResnet50_model.keras')
        self.face_cascade = cv2.CascadeClassifier('dog_breed_classifier-main/haarcascades/haarcascade_frontalface_alt.xml')
        # Move ResNet50 model loading outside __init__ if possible
        self.ResNet50_model = ResNet50(weights='imagenet', include_top=False)
        self.dog_names = dog_names = [item[20:-1] for item in sorted(glob("dog_breed_classifier-main/data/dog_images/train/*/"))]
    ...
    def __DogResNet50_predict_breed(self, img_path):
        ...
        dog_breed = self.dog_names[np.argmax(predicted_vector)][6:].lower().replace("_"," ")
        return dog_breed
